...
/*** From source: /home/slade/defi_guardian/translated_output.pml ***/
...
short src_ln1 [] = {
	  0,  34,  36,  38,  39,  37,  41,  41, 
	 35,  44,  44,  46,  48,  49,  47,  51, 
	 51,  45,  54,  54,  56,  58,  59,  57, 
	 61,  61,  55,  64,  64,  66,  68,  69, 
	 70,  67,  72,  72,  65,  74,  33,  75, 
	 33,  75,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "/home/slade/defi_guardian/translated_output.pml", 1, 41 },
	{ "-", 42, 43 }
};
uchar reached1 [] = {
	  0,   1,   1,   1,   0,   0,   1,   1, 
...
short src_ln0 [] = {
	  0,  20,  21,  19,  25,  26,  27,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "/home/slade/defi_guardian/translated_output.pml", 1, 6 },
	{ "-", 7, 8 }
};
uchar reached0 [] = {
	  0,   1,   0,   0,   0,   0,   0,   0, };
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* User */
		...
		((P1 *)pptr(h))->action = 0;
#ifdef VAR_RANGES
		logval("User:id", ((P1 *)pptr(h))->id);
		logval("User:action", ((P1 *)pptr(h))->action);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
{
		now.total_liquidity = 1000;
		now.users[0].balance = 0;
		now.users[0].debt = 0;
		now.users[1].balance = 0;
		now.users[1].debt = 0;
#ifdef VAR_RANGES
		logval("total_liquidity", now.total_liquidity);
		logval("users[0].balance", now.users[0].balance);
		logval("users[0].debt", now.users[0].debt);
		logval("users[1].balance", now.users[1].balance);
		logval("users[1].debt", now.users[1].debt);
#endif
}
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   total_liquidity:	%d\n", now.total_liquidity);
		printf("	(struct users)\n");
	printf("	int    balance:	%d\n", now.users[ 0 ].balance);
	printf("	int    debt:	%d\n", now.users[ 0 ].debt);
	printf("	int    balance:	%d\n", now.users[ 1 ].balance);
	printf("	int    debt:	%d\n", now.users[ 1 ].debt);
}
