...
class SPINParser(TraceParser):
    ...
    def parse_trace(
        ...
        """
        Parse SPIN trail replay output into structured trace steps.
        """
        trace = ExecutionTrace()

        if not trail_path or not os.path.exists(trail_path):
            # Try to extract trace from log file
            return self._parse_log_as_trace(log_path)

        # Try to read trail file directly
        try:
            with open(trail_path, "r", errors="replace") as f:
                trail_content = f.read()
                return self._parse_trail_output(trail_content)
        except Exception:
            return self._parse_log_as_trace(log_path)
